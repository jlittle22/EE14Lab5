...
void LCD_PIN_Init(void){
	...
	// Configure Port A Pins 6, 7, 8, 9, 10, 15 as Alternate Function Mode 10
	GPIOA->MODER &= ~(0xC03FF000); 
	GPIOA->MODER |= 0x802AA000;

	// Configure Port A Pins 6, 7, 8, 9, 10, 15 as Alternate Function
	GPIOA->AFR[0] &= ~(0xFF000000);
	GPIOA->AFR[0] |= 0xBB000000;
	GPIOA->AFR[1] &= ~(0xF0000FFF);
	GPIOA->AFR[1] |= 0xB0000BBB;

	// Port B -- Pins to be configured: 0,1,4,5,9,12,13,14,15 as Alternate Function
	...
	// Port C -- Pins to be configured: 3,4,5,6,7 and 8 as Alternate Function
	...
	// Port D -- Pins to be configured: 8,9,10,11,12,13,14,15 as Alternate Function
...
void LCD_Configure(void){
	...
	// Set configuration register bias to 1/3
	LCD->CR &= ~(LCD_CR_BIAS);
	...
	// Set contrast value to be maximal (111)
	LCD->FCR &= ~(LCD_FCR_CC);
	LCD->FCR |= (LCD_FCR_CC_2 | LCD_FCR_CC_1 | LCD_FCR_CC_0);
	
	// Set the pulse on the period to maximum (111)
	LCD->FCR &= ~(LCD_FCR_PON);
	...
	// Wait for FCRSF flag to go high
	while(!(LCD->SR & LCD_SR_FCRSF)) {}
		
	// Set LCD enable to be high
	LCD->CR |= LCD_CR_LCDEN;
	
	// Wait for LCD to become enabled
  while(!(LCD->SR & LCD_SR_ENS)) {}
		
	// Wait until the booster is ready
...
void LCD_DisplayString(uint8_t* ptr){
	...
	// while there is a valid character at the ith index, write it to LCD
	while(ptr[i]) {
		if (i >= 6) return;
...
void LCD_Display_Name(void){
	  char *lastName = "LITTLE\0";
    LCD_DisplayString((uint8_t*)lastName);
}

void LCD_Display_Milliseconds(uint32_t cs) {
	
	  char *converted[8];
	  uint32_t mins = cs / 60000;
	  uint32_t seconds = (cs % 60000) / 1000;
	  cs = (cs % 1000) / 10;
	
	  sprintf((char *)converted, "%.2d%.2d%.2d", mins, seconds, cs);
	
	  LCD_WriteChar((uint8_t*)converted, 0, 0, 0);
	  LCD_WriteChar((uint8_t*)converted + 1, 0, 1, 1);
	  LCD_WriteChar((uint8_t*)converted + 2, 0, 0, 2);
	  LCD_WriteChar((uint8_t*)converted + 3, 1, 0, 3);
	  LCD_WriteChar((uint8_t*)converted + 4, 0, 0, 4);
	  LCD_WriteChar((uint8_t*)converted + 5, 0, 0, 5);
